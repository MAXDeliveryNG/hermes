...
export class ServiceBus {
     ...
     */
    public createTopic(name: string, options: Azure.ServiceBus.CreateTopicIfNotExistsOptions): Promise<Azure.ServiceBus.Response>;
    /**
     * Create a service bus topic if doesn't exist. 
     * @param {string} name 
     * Topic name
    ...
    createTopic(name: string, options?: Azure.ServiceBus.CreateTopicIfNotExistsOptions): Promise<Azure.ServiceBus.Response> {
        return new Promise<Azure.ServiceBus.Response>((resolve, reject) => {
            if (!options) {
                ...
                this._bus.createTopicIfNotExists(name, options, (error, result, response) => {
                    if (error) reject(error);
                    if (result) resolve(response);
                });
            }
     ...
     */
    public createTopicSubscription(name: string, subscriptionName: string, options: Azure.ServiceBus.CreateSubscriptionOptions): Promise<Azure.ServiceBus.Results.Models.Subscription>;
    /**
     * Create a topic subscription. 
     * @param {string} name 
     * Topic name
     ...
     * @returns {Promise<Azure.ServiceBus.Results.Models.Subscription>} 
     */
    public createTopicSubscription(name: string, subscriptionName: string): Promise<Azure.ServiceBus.Results.Models.Subscription>;

    createTopicSubscription(name: string, subscriptionName: string, options?: Azure.ServiceBus.CreateSubscriptionOptions) {
        return new Promise((resolve, reject) => {
            if (!options) {
                this._bus.createSubscription(name, subscriptionName, (error, result) => {
                    if (error) reject(error);
                    if (result) resolve(result);
                });
            } else {
                this._bus.createSubscription(name, subscriptionName, options, (error, result) => {
                    if (error) reject(error);
                    if (result) resolve(result);
                })
            }
     ...
     */
    listSubscriptions(name: string, options?: Azure.ServiceBus.ListSubscriptionsOptions) {
        return new Promise((resolve, reject) => {
            if (options) {
     ...
     * @returns {Promise<Azure.ServiceBus.Response>}
     */
    sendTopicMessage(name: string, data: Dictionary<any>, options?: TopicMessageOptions): Promise<Azure.ServiceBus.Response> {
        return new Promise<Azure.ServiceBus.Response>((resolve, reject) => {
            const body = JSON.stringify(data);
            const contentType = 'json';
            const message = options ? Object.assign(options, { body, contentType }) : { body, contentType };
     ...
     */
    receiveSubscriptionMessage<T>(name: string, subscriptionName: string, isPeekLock: boolean = false): Promise<ReceivedMessage<T>> {
        return new Promise<ReceivedMessage<T>>((resolve, reject) => {
            this._bus.receiveSubscriptionMessage(name, subscriptionName, { isPeekLock }, (error, message) => {
                if (error) { 
                    reject (error);
                } else {
                    const response = {
